...
const wait = async (media) => new Promise(async (resolve, reject) => {
    ...
    try {
    	...
    	let ecch = () => is_adult ? "Simon" : "Nel"
    	resolve({video: await getBuffer(`https://media.trace.moe/video/${anilist_id}/${encodeURIComponent(filename)}?t=${at}&token=${tokenthumb}`), teks: `${belief()}
~> Ecchi : *${ecch()}*
~> Título en japones : *${title}*
...
~> Temporada  : *${season}*`});
		...
		console.log(e)
		reject(`No se encontro el anime`)
	}
})

const simih = async (text) => {
	try {
		const sami = await fetch(`https://api.zeks.xyz/api/simi?apikey=apivinz&text=${text}`, {method: 'GET'})
...
const banner = cfonts.render(('Fsociety Bot|V2'), {
    font: 'chrome',
    color: 'candy',
    align: 'center',
    gradient: ["red","yellow"],
